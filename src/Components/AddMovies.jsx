...
function AddMovies() {
  ...
  let handleEditMovie = async (id) => {
    try {
      let editmovie = await axios.get(`${Config.api}/api/movie/movies/${id}`);
  ...
  const handleDeleteMovie = async (id) => {
    try {
      let movieDelete = await axios.delete(`${Config.api}/api/movie/movies/${id}`);
      fetchMovieData();
  ...
  let formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (!isMovieEditing) {
          let result = await axios.post(`${Config.api}/api/movie/movies`, values);
          console.log(result);
          fetchMovieData();
          formik.resetForm();
          ...
          let result = await axios.put(
            `${Config.api}/movies/${currentMovie.id}`,
            values
          );
          fetchMovieData();
          formik.resetForm();
    ...
    <>
      <div className="row">
        <table className="table m-4">
          ...
          <tbody>
            {moviesListAll && moviesListAll.map((m, index) => {
              return (
                <tr key={index}>
